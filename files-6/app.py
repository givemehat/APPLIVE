...
from utils import (make_gauge_chart, make_pie_chart, make_comparison_bar,
                   make_probability_bar, save_profile, load_all_profiles,
                   get_ai_advice, generate_pdf_report, PORTFOLIO_ALLOCATIONS,
                   CATEGORY_COLORS)
...
def page_home():
    ...
    with col1:
        st.markdown('<div class="hero-title">Know Your<br>Investment DNA</div>', unsafe_allow_html=True)
        st.markdown('<div class="hero-sub">AI-powered risk profiling in under 2 minutes.<br>Get personalized portfolio advice backed by ML.</div>', unsafe_allow_html=True)
        ...
        with c1:
            st.markdown('<div class="stat-card"><div class="stat-val">1,200+</div><div class="stat-label">Training Samples</div></div>', unsafe_allow_html=True)
        with c2:
            st.markdown('<div class="stat-card"><div class="stat-val">3</div><div class="stat-label">Risk Profiles</div></div>', unsafe_allow_html=True)
        with c3:
            st.markdown('<div class="stat-card"><div class="stat-val">ML+AI</div><div class="stat-label">Powered</div></div>', unsafe_allow_html=True)

    with col2:
        # Feature cards
        features = [
            ("🧠", "Machine Learning Model", "Logistic Regression trained on 1,200+ investor profiles"),
            ("📊", "Visual Analytics", "Gauge, pie, and bar charts for complete clarity"),
            ("🤖", "AI Personalized Advice", "GPT-powered recommendations tailored to you"),
    ...
    # How it works
    st.markdown('<div style="font-size:1.3rem;font-weight:700;color:#ccc;margin-bottom:1rem">How It Works</div>', unsafe_allow_html=True)
    cols = st.columns(4)
    steps = [
...
# ═══════════════════════════════════════════════════════════════════════════════
def page_quiz():
    st.markdown('<div style="font-size:1.6rem;font-weight:700;color:#ccc;margin-bottom:0.3rem">📋 Investment Risk Assessment</div>', unsafe_allow_html=True)
    st.markdown('<div style="color:#555;margin-bottom:1.5rem">Answer honestly — your results are only as good as your inputs.</div>', unsafe_allow_html=True)

    with st.form('quiz_form'):
        # Q1 — Age
        q = QUESTIONS[0]
        st.markdown(f'<div class="question-label">1. {q["label"]}</div>', unsafe_allow_html=True)
        age = st.slider('', min_value=18, max_value=70, value=30, key='age', label_visibility='collapsed')

        st.markdown('---')

        # Q2 — Monthly Income
        q = QUESTIONS[1]
        st.markdown(f'<div class="question-label">2. {q["label"]}</div>', unsafe_allow_html=True)
        income = st.select_slider(
            '', options=[5000, 10000, 20000, 35000, 50000, 75000, 100000, 150000, 200000],
            format_func=lambda x: f'₹{x:,}',
            value=35000, key='income', label_visibility='collapsed'
        )
        ...
        # Q3 — Experience
        q = QUESTIONS[2]
        st.markdown(f'<div class="question-label">3. {q["label"]}</div>', unsafe_allow_html=True)
        experience = st.slider('', min_value=0, max_value=30, value=3, key='exp', label_visibility='collapsed')
        st.caption(f'{"🔰 Beginner" if experience < 2 else "📈 Intermediate" if experience < 7 else "🏆 Expert"} — {experience} year(s)')

        st.markdown('---')

        # Q4 — Risk Tolerance
        q = QUESTIONS[3]
        st.markdown(f'<div class="question-label">4. {q["label"]}</div>', unsafe_allow_html=True)
        risk_tol = st.radio(
            '',
            options=[1, 2, 3, 4, 5],
        ...
        q = QUESTIONS[4]
        st.markdown(f'<div class="question-label">5. {q["label"]}</div>', unsafe_allow_html=True)
        horizon_label = st.selectbox(
            '', options=['< 6 months', '6–12 months', '1–3 years', '3–7 years', '7–15 years', '15+ years'],
            key='horizon', label_visibility='collapsed'
        )
        horizon_map = {
            '< 6 months': 3, '6–12 months': 9, '1–3 years': 24,
            '3–7 years': 60, '7–15 years': 120, '15+ years': 240
        }
        ...
        # Q6 — Financial Goal
        q = QUESTIONS[5]
        st.markdown(f'<div class="question-label">6. {q["label"]}</div>', unsafe_allow_html=True)
        goal = st.radio(
            '',
        ...
        # Q7 — Loss Reaction
        q = QUESTIONS[6]
        st.markdown(f'<div class="question-label">7. {q["label"]}</div>', unsafe_allow_html=True)
        behavior = st.radio(
            '',
...
def page_result():
    ...
    with col1:
        st.markdown(f'<div style="font-size:1.8rem;font-weight:700;color:#ccc">Your Investment Risk Profile</div>', unsafe_allow_html=True)
        st.markdown(f'<br><span class="badge {badge_class}">🎯 {category} Investor</span>', unsafe_allow_html=True)
    with col2:
    ...
    with col_t:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown('<div style="font-weight:600;color:#ccc;margin-bottom:0.8rem">📌 Recommended Allocation</div>', unsafe_allow_html=True)
    ...
    with col_s:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown('<div style="font-weight:600;color:#ccc;margin-bottom:0.8rem">👤 Your Profile Summary</div>', unsafe_allow_html=True)
        summary_items = [
    ...
    # ── AI Advice ──
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div style="font-weight:600;color:#ccc;margin-bottom:0.8rem">🤖 AI Financial Advisor Says</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="advice-box">{advice.replace(chr(10), "<br><br>")}</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
def page_admin():
    st.markdown('<div style="font-size:1.6rem;font-weight:700;color:#ccc;margin-bottom:1rem">🛠️ Admin Dashboard</div>', unsafe_allow_html=True)
    ...
    if df.empty:
        ...
        color_map = {'Conservative': '#4CAF50', 'Moderate': '#FF9800', 'Aggressive': '#F44336'}
        ...
        if len(df) > 1:
            fig_line = px.line(
                df.sort_values('id'), x='id', y='risk_score',
        ...
        # Table
        st.markdown('<div style="font-weight:600;color:#ccc;margin:1rem 0 0.5rem">All Profiles</div>', unsafe_allow_html=True)
        display_df = df.copy()
