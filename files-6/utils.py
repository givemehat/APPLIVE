...
def make_gauge_chart(score: float, category: str):
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            'axis': {'range': [0, 100], 'tickcolor': '#aaa', 'tickfont': {'color': '#aaa'}},
        ...
        title={'text': f"<b>Risk Score</b><br><span style='color:{color};font-size:18px'>{category}</span>",
...
def make_comparison_bar(user_data: dict, category: str):
    ...
    for m in metrics:
        ...
        avg_norm_cons.append((avg.get('Conservative', {}).get(m, lo) - lo) / (hi - lo) * 100)
        avg_norm_mod.append((avg.get('Moderate', {}).get(m, lo) - lo) / (hi - lo) * 100)
        avg_norm_agg.append((avg.get('Aggressive', {}).get(m, lo) - lo) / (hi - lo) * 100)

    x_labels = [labels_map[m] for m in metrics]

    fig = go.Figure()
    fig.add_trace(go.Bar(name='You', x=x_labels, y=user_norm,
                         marker_color=CATEGORY_COLORS[category], opacity=0.9))
    fig.add_trace(go.Bar(name='Avg Conservative', x=x_labels, y=avg_norm_cons,
                         marker_color='#4CAF50', opacity=0.4))
    fig.add_trace(go.Bar(name='Avg Moderate', x=x_labels, y=avg_norm_mod,
                         marker_color='#FF9800', opacity=0.4))
    fig.add_trace(go.Bar(name='Avg Aggressive', x=x_labels, y=avg_norm_agg,
                         marker_color='#F44336', opacity=0.4))
    ...
    fig.update_layout(
        ...
        title=dict(text='Your Profile vs. Category Averages', font=dict(color='white', size=14))
    )
    return fig
...
def make_probability_bar(prob_dict: dict):
    ...
    fig.update_layout(
        ...
        height=280,
        margin=dict(l=40, r=20, t=40, b=40),
        title=dict(text='Model Confidence per Category', font=dict(color='white', size=14))
    )
    return fig
...
Be direct, warm, and practical. No jargon."""
        ...
        response = client.chat.completions.create(
            model='gpt-3.5-turbo',
            messages=[
                {'role': 'system', 'content': 'You are a certified financial advisor who gives clear, actionable advice.'},
...
def generate_pdf_report(user_data: dict, category: str, score: float,
                         prob_dict: dict, advice: str) -> bytes:
    try:
        ...
        class PDF(FPDF):
            def header(self):
                ...
                self.cell(0, 15, 'AI Investment Risk Profile Report', ln=True, align='C')
                self.set_font('Helvetica', '', 10)
                self.set_text_color(160, 160, 160)
                self.cell(0, 8, f"Generated: {datetime.now().strftime('%d %B %Y, %H:%M')}", ln=True, align='C')
                self.ln(5)
        ...
        # Category badge
        cat_color = {'Conservative': (76, 175, 80), 'Moderate': (255, 152, 0), 'Aggressive': (244, 67, 54)}
        r, g, b = cat_color[category]
        pdf.set_fill_color(r, g, b)
        pdf.set_text_color(255, 255, 255)
        pdf.set_font('Helvetica', 'B', 16)
        pdf.cell(0, 12, f'  Risk Category: {category}  |  Risk Score: {score}/100', ln=True, align='C', fill=True)
        pdf.ln(6)
        ...
        pdf.cell(0, 6, 'Disclaimer: This report is AI-generated for educational purposes only. Consult a SEBI-registered advisor before investing.', ln=True, align='C')
